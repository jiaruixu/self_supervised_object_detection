...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset', help='path to dataset folder', default='./dataset/', type=str)
    parser.add_argument('--image-list', dest='image_list', help='text file with path to images relative to dataset folder', type=str)
    parser.add_argument('--output-name', dest='output_name', help='filename for the output text file', type=str)
    parser.add_argument('--output-dir', dest='output_dir', help='path to output folder', default='/root/rrc_detection/output', type=str)
    parser.add_argument('--iter', dest='iter', help='iter of pretrained model', default='60000', type=str)
    parser.add_argument('--model-path', dest='model_path', help='path to model folder',
                        default=os.path.join('models','VGGNet','KITTI','RRC_2560x768_kitti_4r4b_max_size'), type=str)
    parser.add_argument('--model-name', dest='model_name', help='name of caffemodel file without _iter_<iter number>.caffemodel suffix',
                        default='VGG_KITTI_RRC_2560x768_kitti_4r4b_max_size', type=str)
    parser.add_argument('--gpus', dest='gpu_id', help='GPU devices to evaluate with',
                        default='0', type=str)
    parser.add_argument('--draw-results', dest='draw', help='use ImageDraw to draw bboxes over the image and write to the output dir',
                        default=0, type=int)
...
def main(args):
    ...
    model_weights = os.path.join(caffe_root, args.model_path, args.model_name + '_iter_' + args.iter +'.caffemodel')
    voc_labelmap_file = os.path.join(caffe_root,'data','KITTI-car','labelmap_voc.prototxt')
